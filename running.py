...
def train_round_fedpac(args, global_model, local_clients, rnd, **kwargs):
    ...
    local_acc1 = []
    local_acc2 = []
    agg_weight = []  # aggregation weights for f 保存数据量
    avg_weight = []  # aggregation weights for g
    sizes_label = []  # 每个类别在客户端中的数量
    local_protos = []  # 保存局部质心[{类别:特征均值(1,128)}，{}]
    ...
    if rnd <= args.epochs:
        for idx in idx_users:
            ...
            Hs.append(copy.deepcopy(h))  #(c,num_classes,128)
            ## 2.local training
            local_epoch = args.local_epoch
        ...
        # update global prototype {类别:特征均值(1,128)}
        ...
        if agg_g and rnd < args.epochs:
            ...
            for idx in idx_users:
                ...
                idxx += 1
                # 获取第一次学习权重
                if local_client.personal_dict is None:
                    local_client.personal_dict=local_client.local_model.state_dict()
