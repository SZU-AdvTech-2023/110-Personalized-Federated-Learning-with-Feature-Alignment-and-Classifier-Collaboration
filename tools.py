...
# 返回数据集每类每个特征的均值，即返回{类别:行向量(128个特征的值，均值)}
...
def get_head_agg_weight(num_users, Vars, Hs, *args, **kwargs):
    ...
    d = Hs[0].shape[1]  # dimension of feature representation 128
    avg_weight = []
    for i in range(num_users):
        ...
        # 客户端两两组合如3，(0,1)(0,2)(1,2)
        for j1, j2 in pairwise(tuple(range(num_users))):
            h_j1 = Hs[j1]
            h_j2 = Hs[j2]
            h = torch.zeros((d, d), device=device)  # (128,128)
        ...
        # 1.9之后使用torch.linalg.eig而不是torch.eig
        # evals, evecs = torch.eig(torch.tensor(p_matrix), eigenvectors=True)
        # 得到的值为复杂类型的float，只需实部即取real
